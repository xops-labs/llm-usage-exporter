...
public sealed class ProviderFixtureGoldenMetricsTests
{
    private const string AzureAccountResourceId =
        "subscriptions/sub-1/resourceGroups/rg-1/providers/Microsoft.CognitiveServices/accounts/aoai-fixture";

    [Fact]
    public async Task ProviderPayloadFixtures_MapToGoldenMetricOutput()
    {
        ...
        foreach (ProviderScenario scenario in await LoadProviderScenariosAsync())
        {
        ...
        foreach (string expectedLine in LoadGoldenMetricLines())
        {
            Assert.Contains(expectedLine, metrics);
        }
    }

    private static async Task<IReadOnlyList<ProviderScenario>> LoadProviderScenariosAsync()
    {
        return
        [
    ...
    private static async Task<ProviderScenario> LoadOpenAiScenarioAsync()
    {
        OpenAiUsageProvider provider = CreateOpenAiProvider(request =>
        {
            ...
            return path.Contains("/costs", StringComparison.OrdinalIgnoreCase)
                ? JsonFixtureResponse("openai-costs.json")
                : JsonFixtureResponse("openai-completions-usage.json");
        });
    ...
    private static async Task<ProviderScenario> LoadAzureOpenAiScenarioAsync()
    {
        AzureOpenAiUsageProvider provider = CreateAzureOpenAiProvider(request =>
        {
            ...
            {
                return JsonFixtureResponse("azure-cost-query.json");
            }

            return JsonFixtureResponse("azure-monitor-metrics.json");
        });
        ...
        IReadOnlyCollection<LlmCostBucket> costs = await provider.GetCostsAsync(
            DateTimeOffset.Parse("2024-03-09T00:00:00Z"),
            DateTimeOffset.Parse("2024-03-10T00:00:00Z"),
            CancellationToken.None);
    ...
    private static async Task<ProviderScenario> LoadAnthropicScenarioAsync()
    {
        AnthropicUsageProvider provider = CreateAnthropicProvider(request =>
        {
            ...
            return path.Contains("cost_report", StringComparison.OrdinalIgnoreCase)
                ? JsonFixtureResponse("anthropic-cost-report.json")
                : JsonFixtureResponse("anthropic-messages-usage.json");
        });
    ...
    private static async Task<ProviderScenario> LoadGeminiScenarioAsync()
    {
        GeminiUsageProvider provider = CreateGeminiProvider(request =>
        {
            ...
            {
                return JsonFixtureResponse("gemini-billing-costs.json");
            }

            if (url.Contains("request_type = \"input\"", StringComparison.Ordinal))
            {
                return JsonFixtureResponse("gemini-input-tokens.json");
            }

            if (url.Contains("request_type = \"output\"", StringComparison.Ordinal))
            {
                return JsonFixtureResponse("gemini-output-tokens.json");
            }

            if (url.Contains("request_count", StringComparison.Ordinal))
            {
                return JsonFixtureResponse("gemini-request-count.json");
            }
    ...
    private static async Task<ProviderScenario> LoadBedrockScenarioAsync()
    {
        BedrockUsageProvider provider = CreateBedrockProvider(request =>
        {
            ...
            {
                return JsonFixtureResponse("bedrock-cost-explorer.json", "application/x-amz-json-1.1");
            }
            ...
            {
                return XmlFixtureResponse("bedrock-cloudwatch-input.xml");
            }

            if (body.Contains("OutputTokenCount", StringComparison.Ordinal))
            {
                return XmlFixtureResponse("bedrock-cloudwatch-output.xml");
            }

            if (body.Contains("Invocations", StringComparison.Ordinal))
            {
                return XmlFixtureResponse("bedrock-cloudwatch-invocations.xml");
            }
    ...
    private static BedrockUsageProvider CreateBedrockProvider(Func<HttpRequestMessage, HttpResponseMessage> responseFactory)
    {
        var bedrockOptions = new BedrockOptions
        {
            ...
            ModelIds = ["anthropic.claude-fixture-v1:0"],
    ...
    private static IReadOnlyList<string> LoadGoldenMetricLines()
    {
        return File.ReadAllLines(FixturePath("golden-metrics", "canonical-provider-metrics.prom"), Encoding.UTF8)
